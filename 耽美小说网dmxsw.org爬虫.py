...
'''
http://www.dmxsw.org/cycs/10268/29.html
'''
book_title = []
chapter_url_list = []
charset = "gb2312"
...
def getChapters():
    ...
    book_url = 'http://dmxsw.org/cycs/11107.html'
    base_url = 'http://www.dmxsw.org'

    session = requests.Session()
    response_1 = session.get(book_url, headers=headers)
    response_1.encoding = charset

    #print(response_1.text)

    book_title_regex = '<div class="infos">\n|.<h1>(.*)</h1>'
...
def getContent(url):
    ...
    response_2.encoding = charset

    #print(response_2.text)

    article_regx = '<div class="read_article">((\n|.)*)</div>'
    article = re.findall(article_regx, response_2.text)
    article = str(article[0])
    article_index = article.find("</div>")
    content = str(article[0:article_index]).replace("\ufffd","*").replace("\u7947","祇").replace('<p>', '\n').replace('</p>', '\n').replace("\\u3000","").strip("'(\\r\\n ')")
...
def export():
    ...
    with open(save_path, 'a+', encoding=charset) as f:
